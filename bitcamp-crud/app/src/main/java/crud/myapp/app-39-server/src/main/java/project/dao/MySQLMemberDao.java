...
public class MySQLMemberDao implements MemberDao {
  ...
  public int update(Member member) {
    try (PreparedStatement stmt = con.prepareStatement("update project_member set " + "name=?, "
        + "age=?, " + "height=?, " + "weight=?, " + "gender=?, " + "leftEye=?, " + "rightEye=?, "
        + "handPhone=?, " + "password=sha1(?) " + "where member_no=?")) {
