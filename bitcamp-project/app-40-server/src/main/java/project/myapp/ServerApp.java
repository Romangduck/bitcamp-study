...
public class ServerApp {
  ...
  public ServerApp(int port) throws Exception {
    ...
    this.opnionDao = new MySQLBoardDao(ds, 1);
    this.boardDao = new MySQLBoardDao(ds, 2);
    this.homeworkDao = new MySQLBoardDao(ds, 3);
  ...
  private void prepareMenu() {
    ...
    boardMenu.add(new Menu("등록", new BoardAddListener(opnionDao, ds)));
    boardMenu.add(new Menu("목록", new BoardListListener(opnionDao)));
    boardMenu.add(new Menu("조회", new BoardDetailListener(opnionDao, ds)));
    boardMenu.add(new Menu("변경", new BoardUpdateListener(opnionDao, ds)));
    boardMenu.add(new Menu("삭제", new BoardDeleteListener(opnionDao, ds)));
    mainMenu.add(boardMenu);

    MenuGroup opnionMenu = new MenuGroup("자유 게시판");
    opnionMenu.add(new Menu("등록", new BoardAddListener(boardDao, ds)));
    opnionMenu.add(new Menu("목록", new BoardListListener(boardDao)));
    opnionMenu.add(new Menu("조회", new BoardDetailListener(boardDao, ds)));
    opnionMenu.add(new Menu("변경", new BoardUpdateListener(boardDao, ds)));
    opnionMenu.add(new Menu("삭제", new BoardDeleteListener(boardDao, ds)));
    mainMenu.add(opnionMenu);

    MenuGroup homeworkMenu = new MenuGroup("숙제 게시판");
    homeworkMenu.add(new Menu("등록", new BoardAddListener(homeworkDao, ds)));
    homeworkMenu.add(new Menu("목록", new BoardListListener(homeworkDao)));
    homeworkMenu.add(new Menu("조회", new BoardDetailListener(homeworkDao, ds)));
    homeworkMenu.add(new Menu("변경", new BoardUpdateListener(homeworkDao, ds)));
    homeworkMenu.add(new Menu("삭제", new BoardDeleteListener(homeworkDao, ds)));
