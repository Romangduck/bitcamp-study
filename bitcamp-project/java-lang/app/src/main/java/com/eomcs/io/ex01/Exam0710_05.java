...
public class Exam0710_05 {
  ...
  public static void main(String[] args) throws Exception {
    ...
    File dir = new File("."); // 현재 폴더의 정보를 준비
    System.out.println(dir.getCanonicalPath()); // 현재 폴더가 어떤 폴더인지 확인.
  ...
  static void printList(File dir) {
    ...
    for (File file : files) {
      if (file.isDirectory() && !file.isHidden()) {
        System.out.printf("%s/\n", file.getName()); // 디렉토리의 이름을 출력하고
        printList(file); // 그 디렉토리의 하위파일과 폴더를 출력
      } else if (file.isFile()) {
        System.out.printf("%s\n", file.getName());
      }
    }
