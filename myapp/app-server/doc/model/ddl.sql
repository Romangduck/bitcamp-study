...
CREATE TABLE `edu_lect` (
  ...
  `sdt`     <데이터 타입 없음> NOT NULL COMMENT '시작일', -- 시작일
  `edt`     <데이터 타입 없음> NOT NULL COMMENT '종료일', -- 종료일
  `price`   <데이터 타입 없음> NOT NULL COMMENT '수강료', -- 수강료
...
CREATE TABLE `edu_classroom_photo` (
  `cpno`     <데이터 타입 없음> NOT NULL COMMENT '강의실사진번호', -- 강의실사진번호
  `crno`     INTEGER            NOT NULL COMMENT '강의실번호', -- 강의실번호
  `filename` <데이터 타입 없음> NOT NULL COMMENT '사진파일명' -- 사진파일명
)
COMMENT '강의실사진';
...
ALTER TABLE `edu_classroom_photo`
  MODIFY COLUMN `cpno` <데이터 타입 없음> NOT NULL AUTO_INCREMENT COMMENT '강의실사진번호';
...
CREATE TABLE `edu_employ_type` (
  ...
  `title` VARCHAR(50) NOT NULL COMMENT '고용형태명' -- 고용형태명
)
COMMENT '고용형태';
